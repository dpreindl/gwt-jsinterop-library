...
public class WebComponentsUtils {
    ...
    public static class WebcomponentsResolover {
         ...
         */
        @JsProperty(name = "WebComponents", namespace = JsPackage.GLOBAL)
     ...
     *
     * @param function The function which should be executed after the given amount of milliseconds
     * @param milliseconds The milliseconds to wait before the given function is executed.
     */
    @JsMethod(name = "setTimeout", namespace = JsPackage.GLOBAL)
    public static native void setTimeout(CallbackFunction function, int milliseconds);
     ...
     * </pre>
     *
     * @param element The webcomponent element
     * @param f The function to execute as soon as the element is upgraded
     */
    public static void tryWebComponentMethod(WebComponentElement element, CallbackFunction f) {
        if (element != null) {
            if (element.getIs() != null) {
                f.call();
            } else {
                setTimeout(() -> tryWebComponentMethod(element, f), 50);
            }
        }
    }
